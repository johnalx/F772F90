    ...
    CHARACTER (LEN=50), PARAMETER :: blank = ' '
    CHARACTER (LEN= 1), PARAMETER :: tab = CHAR(9)

    CHARACTER (LEN= 1)       :: ch
    CHARACTER (LEN=50)       :: prog_unit_name = ' ', case_expr
    CHARACTER (LEN= 5)       :: lab
    CHARACTER (LEN=30)       :: text, vtype, id
    CHARACTER (LEN=140)      :: statement
    CHARACTER (LEN= 8)       :: date
    CHARACTER (LEN=10)       :: time
    INTEGER                  :: iostatus, pos, count, last, n_marks, pos1(20),  &
        pos2(20), indent, i, i1, i2, &
        numb_arg, i3, i4, path_len, lab_length, length
    ...
    SUBROUTINE process_file(f77_name, f90_name)
    !   Reads f77 file and converts it into f90
    CHARACTER(LEN=255), INTENT(IN)  :: f77_name, f90_name
    ...
    IF (head % text(1:1) == 'C' .OR. head % text(1:1) == 'c' .OR.   &
        head % text(1:1) == '*' .OR. head % text(1:1) == 'D' ) THEN
    head % text(1:1) = '!'
    ...
    DO
        ...
        IF (current % text(1:1) == 'C' .OR. current % text(1:1) == 'c' .OR.   &
            current % text(1:1) == '*' .OR. current % text(1:1) == 'D' ) THEN
    ...
    DO
        ...
        IF (INDEX(current % text, 'FUNCTION') == 0) THEN
            ...
            IF (pos > 0) THEN
                ! Any existing '::' indicates F90 declaration.
                if( index(current%text, '::')==0 ) then

                    asterisk = INDEX(current % text(pos-1:pos), '*') > 0
                    IF (.NOT. asterisk) THEN
                        IF (pos /= 11) THEN
                            ! Check for F90 style declarations "integer(4)"
                            if( current%text(pos-1:pos-1)=='(') then
                                ! do nothing
                            else
    ...
    outer_loop: DO
        ...
        DO
            ...
            CASE (0, 3)
                current % text = TRIM(current % text) // ', INTENT(INOUT)'
            CASE (1)
...
100     current => end_prog_unit
    ...
    IF (ch /= ' ') WRITE(9, *)
    WRITE(9, '("! Code converted by ", a," using TO_F90 tool.")') id(1:LEN_TRIM(id))
